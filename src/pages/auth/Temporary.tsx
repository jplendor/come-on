...
const Temporary = (): JSX.Element => {
  ...
  return (
    <>
      <Link href="/auth/test-needed-token">로그인권한이 필요한 링크</Link>
      <br />
      <Link href="/auth/test-needed-token2">로그인권한이 필요한 링크2</Link>
      <br />
      <Link href="/user/my-page">마이페이지 가기</Link>
      <br />
      <Link href="/auth/login">로그인 페이지 가기</Link> <br />
      <Link href="/">홈</Link>
      ...
      {isloggedin ? (
        ...
        <p>
          <Link href="/auth/login">로그인 페이지 가기(비로그인)</Link>
        </p>
      )}
    </>
  )
